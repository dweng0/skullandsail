...
export class MultiplayerManager {
  ...
  async createMultiplayerSession(): Promise<string> {
    try {
      this.sessionCode = Math.random().toString(36).substr(2, 8).toUpperCase();
      this.playerId = 1;
      this.isHost = true;
  ...
  async connectPeer(remotePeerId: number): Promise<RTCPeerConnection> {
    const peerConnection = new RTCPeerConnection(MultiplayerManager.ICE_CONFIG);
  ...
  private handleRemoteEmote(_remotePeerId: number, message: any): void {
    ...
    console.log(`Player ${message.playerId} used emote: ${message.emoteType}`);
  }
